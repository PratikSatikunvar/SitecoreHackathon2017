...
function Create-Config
{
    ...
    if ($jsonFile)
    {
        ...
        Add-Member -InputObject $config -Name TemplateTDSProjectGuid -Value $jsonFile.config.TemplateTDSProjectGuid -MemberType NoteProperty
        ...
        $tdsProjectGuid = [guid]::NewGuid().toString().toUpper()
        Add-Member -InputObject $config -Name TDSProjectGuid -Value $tdsProjectGuid -MemberType NoteProperty
...
function Rename-Module
{
    ...
    $newStartPath = Join-Path -Path $StartPath -ChildPath "code"
    ...
    Update-FileContent -StartPath "$StartPath" -OldValue $config.TemplateTDSProjectGuid -NewValue $config.TDSProjectGuid -FileExtensionsRegex $config.fileExtensionsToUpdateProjectGuidsRegex
...
function Add-Projects
{
    ...
    $ModuleCodePath = Join-Path -Path $ModulePath -ChildPath "code"
    $ModuleTDSPath = Join-Path -Path $ModulePath -ChildPath "tds"
    ...
    # Search in the new module folder for scproj files - TDS Projects and add those to the solution.
    Get-ChildItem -File -Path $ModulePath -Filter "*$scprojExtension" -Recurse | ForEach-Object { $moduleNameInterface.AddFromFile("$($_.FullName)")}
